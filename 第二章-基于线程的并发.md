...
要确定哪个线程在运行，我们创建目标函数，然后导入time模块以开始两秒钟的执行挂起：  
...
threading模块是创建和管理线程的优选形式。每个线程都由
...
当属于并发线程的两个或者更多个操作访问共享内存时，这些线程中的至少有一个线程拥有改变数据状态而不需要用到合适的异步机制、竞争条件，而且它还可能产生无效代码的执行，bug以及未预料的行为。获得竞争条件的嘴角单方式是使用锁。锁的概念是很简单的；当一个线程想要访问局部共享内存时，它必须请求一个局部优先锁以使用这部分内存。此外，对于这种情况，当操作完成后，线程必须
...
## These two functions run n in separate 这两个函数分别独立的线程中运行n，并且调用Box的方法
...
## Thread synchronization with semaphores 使用信号量实现的线程异步
...
事件是用做线程间通信的对象。一个线程会在其他线程输出时等待一个信号。基本上，
